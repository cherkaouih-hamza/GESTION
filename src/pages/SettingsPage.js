...
const SettingsPage = () => {
  ...
  const [settings, setSettings] = useState({
    ...
    poles: { // Add poles settings
      availablePoles: [
        { id: 'tech', name: 'التقنية', isActive: true },
        { id: 'media', name: 'الإعلام', isActive: true },
        { id: 'pedagogical', name: 'ال Pedagogical', isActive: true },
        { id: 'administrative', name: 'الإدارية', isActive: true },
        { id: 'pedagogique', name: 'ال Pedagogique', isActive: false }
      ]
    }
  });
  ...
  // Function to handle changes to poles
  const handlePoleChange = (id, field, value) => {
    setSettings(prev => ({
      ...prev,
      poles: {
        ...prev.poles,
        availablePoles: prev.poles.availablePoles.map(pole =>
          pole.id === id ? { ...pole, [field]: value } : pole
        )
      }
    }));
  };

  // Function to add a new pole
  const addPole = () => {
    const newPole = {
      id: `pole_${Date.now()}`,
      name: 'الاسم الجديد',
    ...
    setSettings(prev => ({
      ...
      poles: {
        ...prev.poles,
        availablePoles: [...prev.poles.availablePoles, newPole]
      }
    }));
  };

  // Function to remove a pole
  const removePole = (id) => {
    setSettings(prev => ({
      ...prev,
      poles: {
        ...prev.poles,
        availablePoles: prev.poles.availablePoles.filter(pole => pole.id !== id)
      }
    }));
  };
  ...
  const renderPoleSettings = () => (
    <div className="space-y-8">
      <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-200">
        <div className="flex items-center mb-6">
          <div className="w-2 h-8 bg-orange-600 rounded-full ml-3"></div>
          <h3 className="text-xl font-semibold text-gray-800">إعدادات الأقطاب</h3>
        </div>

        <div className="mb-6 bg-blue-50 border border-blue-200 rounded-xl p-4">
          <p className="text-sm text-blue-800 text-right">
            <svg className="w-5 h-5 inline ml-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
              <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd"></path>
            </svg>
            هذه هي الأقطاب التي يمكن اختيارها عند إنشاء المهام
          </p>
        </div>

        <div className="space-y-4">
          {settings.poles.availablePoles.map((pole) => (
            <div key={pole.id} className="p-4 bg-gray-50 rounded-xl border border-gray-200 flex items-center justify-between">
              <div className="flex-1">
                <input
                  type="text"
                  value={pole.name}
                  onChange={(e) => handlePoleChange(pole.id, 'name', e.target.value)}
              ...
              <div className="flex items-center space-x-4 space-x-reverse mr-4">
                <label className="flex items-center cursor-pointer">
                  <div className="relative">
                    <input
                      type="checkbox"
                      checked={pole.isActive}
                      onChange={(e) => handlePoleChange(pole.id, 'isActive', e.target.checked)}
                      className="sr-only"
                    />
                    <div className={`block w-10 h-6 rounded-full ${pole.isActive ? 'bg-orange-600' : 'bg-gray-300'}`}></div>
                    <div className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${pole.isActive ? 'transform translate-x-4' : ''}`}></div>
                  </div>
                  <span className="mr-3 text-sm text-gray-700">{pole.isActive ? 'مفعل' : 'غير مفعل'}</span>
                </label>
                <button
        ...
        <div className="mt-6">
          <button
            onClick={addPole}
            className="px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500 flex items-center"
          >
            <svg className="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="mt-8 flex justify-end">
          <button
            onClick={() => handleSaveSettings('poles')}
            className="px-8 py-3 bg-gradient-to-r from-orange-600 to-orange-700 text-white rounded-xl hover:from-orange-700 hover:to-orange-800 focus:outline-none focus:ring-2 focus:ring-orange-500 shadow-md hover:shadow-lg transition-all duration-300 flex items-center"
          >
            <svg className="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path>
            </svg>
      ...
      <div className="py-6 px-4 sm:px-6 lg:px-8 settings-page">
        ...
        <div className="tab-navigation">
          ...
          <button
            ...
            className={`tab-item ${activeTab === 'poles' ? 'active' : ''}`}
          >
            الأقطاب
          </button>
          <button
        ...
        {activeTab === 'poles' && renderPoleSettings()}
        {activeTab === 'info' && renderSystemInfo()}
        {activeTab === 'database-test' && <DatabaseTestComponent />}
      </div>
    </DashboardLayout>
  );
};
